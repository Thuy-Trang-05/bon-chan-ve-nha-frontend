/**
 * TrangAdmin.jsx — Giao diện Trang quản trị hệ thống "Bốn Chân Về Nhà"
 * -----------------------------------------------------------------
 * Trang này chỉ dành cho tác nhân Quản trị viên (Admin) — cần kiểm tra
 * role trong JWT trước khi hiển thị (đã trình bày ở mục 2.3.5), chưa
 * xử lý ở file preview này.
 *
 * Các use case liên quan ở Chương 3 (quyền hạn Admin, mục 3.1):
 *   - Kiểm duyệt tin đăng (gỡ tin vi phạm khỏi hiển thị, không phải
 *     duyệt trước khi đăng — vì tin đăng hiển thị công khai ngay khi
 *     tạo, đúng như đã trình bày ở phần "Cách hoạt động" của Trang chủ)
 *   - Quản lý tài khoản người dùng (khóa / mở khóa)
 *   - Xử lý báo cáo vi phạm (từ nút "Báo cáo tin đăng" ở trang Chi tiết)
 *
 * Ghi chú tích hợp API thật:
 *   - GET /api/admin/thong-ke, GET /api/admin/tin-dang,
 *     GET /api/admin/nguoi-dung, GET /api/admin/bao-cao
 *   - PATCH /api/tin-dang/{id} { trang_thai: "da_an" }  → gỡ tin
 *   - PATCH /api/nguoi-dung/{id} { dang_hoat_dong: false } → khóa tài khoản
 *   - PATCH /api/bao-cao/{id} { trang_thai: "da_xu_ly" }
 */
...
// Hồ sơ đối chiếu người đăng — dùng cho UC-17 (Đối chiếu hồ sơ khi xử lý
// tin đăng): mỗi khi Admin xử lý 1 tin ở UC-16, hệ thống tra cứu lịch sử
// vi phạm của người đăng để hỗ trợ ra quyết định (đúng luồng ngoại lệ
// "[2b] Tài khoản có lịch sử vi phạm chưa xử lý" đã mô tả ở Chương 3).
...
export default function TrangAdmin() {
  ...
  const [tinDoiChieu, setTinDoiChieu] = useState(null); // UC-17: tin đang được đối chiếu hồ sơ
  ...
  // UC-17: lịch sử báo cáo trước đó của người đang bị đối chiếu hồ sơ —
  // trước đây tra bằng HO_SO_NGUOI_DANG viết cứng theo tên, nay tra
  // trực tiếp từ danhSachBaoCao thật theo đúng id người bị báo cáo.
  ...
  // "Gỡ tin vi phạm": vừa ẩn tin đăng thật (UC-16), vừa đánh dấu báo cáo đã xử lý
        ...
        return (
          <div className="fixed inset-0 z-40 bg-black/40 flex items-center justify-center px-5 py-8 overflow-y-auto" onClick={() => setTinDoiChieu(null)}>
            <div className="bg-white rounded-2xl max-w-lg w-full p-6 my-auto" onClick={(e) => e.stopPropagation()}>
              ...
              {/* Cảnh báo nếu tài khoản có báo cáo chưa xử lý — đúng luồng
                 ngoại lệ [2b] đã mô tả ở Chương 3 */}
              ...
              {/* Lịch sử báo cáo liên quan tới người đăng này */}
