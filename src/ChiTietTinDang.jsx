/**
 * ChiTietTinDang.jsx — Giao diện trang "Chi tiết tin đăng" + bản đồ
 * -----------------------------------------------------------------
 * Trang này phục vụ các use case liên quan ở Chương 3:
 *   - UC-10: Xem thông tin bài đăng      (tác nhân: Khách, Người dùng)
 *   - UC-11: Xem vị trí tin đăng trên bản đồ (thu nhỏ về đúng 1 tin)
 *   - Báo cáo tin vi phạm (mở modal ngay tại trang này)
 *
 * Ghi chú tích hợp API thật:
 *   - Dữ liệu tin đăng lấy từ GET /api/tin-dang/{id}
 *   - Khối bản đồ hiện là bản rút gọn (1 điểm ghim tĩnh) để xem giao
 *     diện. Trong dự án thật thay bằng component bản đồ của thư viện bản đồ tương tác (kết hợp OpenStreetMap)
 *     với center = [tin.lat, tin.lng] (thư viện bản đồ tương tác (kết hợp OpenStreetMap) không chạy được
 *     trong môi trường xem trước này).
 *   - Nút "Chỉ đường trên Google Maps" dùng link thật, hoạt động ngay
 *     cả ở bản xem trước, không cần chờ tích hợp thêm.
 *   - Nút "Gọi điện" dùng thẻ <a href="tel:...">, cũng là chức năng
 *     thật của trình duyệt/điện thoại.
 */
...
// Dữ liệu mẫu — sẽ thay bằng dữ liệu thật lấy từ GET /api/tin-dang/{id}
const TIN_MAU = {
  id: 1,
...
// UC-19: Gợi ý tin đăng có ảnh tương đồng (phạm vi mở rộng, tác nhân:
// Khách, Người dùng). Độ giống được tính từ khoảng cách Hamming giữa
// perceptual hash của ảnh đang xem và ảnh các tin khác — kỹ thuật đã
// trình bày ở mục 2.3.6. Độ giống càng cao (khoảng cách Hamming càng
// nhỏ) thì càng được ưu tiên hiển thị lên đầu danh sách gợi ý.
      ...
      <div className="max-w-5xl mx-auto px-5 py-8 grid lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-white rounded-2xl border border-[#F0E4C4] p-5">
            ...
            {/* Đây CHỈ là hình minh họa vị trí để xem bố cục giao diện —
               không phải bản đồ thật, vì môi trường xem trước không tải
               được bản đồ thật.

               CODE THẬT của dự án phải dùng thư viện bản đồ kết hợp
               OpenStreetMap (đúng như đã chọn và giải thích lý do ở mục
               2.3.4 bài tiểu luận — Google Maps cần tài khoản trả phí,
               không phù hợp). Đoạn code mẫu để thay khối div bên dưới,
               Claude đã gửi riêng ở khung chat, không đặt trong file
               này để tránh công cụ xem trước từ chối tải artifact. */}
