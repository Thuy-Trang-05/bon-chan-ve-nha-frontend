/**
 * DangNhapDangKy.jsx — Giao diện trang "Đăng nhập / Đăng ký"
 * -----------------------------------------------------------------
 * Trang này phục vụ 2 use case ở Chương 3:
 *   - UC-01: Đăng ký tài khoản   (tác nhân: Khách)
 *   - UC-02: Đăng nhập           (tác nhân: Khách)
 * Gộp chung 1 file vì cùng một luồng xác thực và dùng chung bố cục —
 * người dùng chuyển qua lại giữa 2 chế độ bằng tab phía trên form,
 * không cần điều hướng sang trang khác.
 *
 * Ghi chú tích hợp API thật (Backend FastAPI):
 *   - Đăng ký:  POST /api/auth/dang-ky
 *               body: { email, mat_khau, ho_ten, so_dien_thoai }
 *   - Đăng nhập: POST /api/auth/dang-nhap
 *               body: { email, mat_khau }
 *               trả về { access_token, token_type, nguoi_dung }
 *   - Sau khi đăng nhập/đăng ký thành công, lưu access_token (ví dụ
 *     vào localStorage) rồi điều hướng về Trang chủ bằng
 *     useNavigate() của react-router-dom, ví dụ:
 *
 *       const res = await fetch(`${API_BASE_URL}/api/auth/dang-nhap`, {
 *         method: "POST",
 *         headers: { "Content-Type": "application/json" },
 *         body: JSON.stringify({ email, mat_khau: matKhau }),
 *       });
 *       const du_lieu = await res.json();
 *       if (!res.ok) throw new Error(du_lieu.detail);
 *       localStorage.setItem("access_token", du_lieu.access_token);
 *       navigate("/");
 *
 *   File này được viết độc lập để xem trước giao diện, chưa gắn
 *   Router nên nút điều hướng vẫn đang dùng onClick + console.log.
 */
...
export default function DangNhapDangKy() {
  ...
  // Vào bằng /dang-ky thì mở sẵn tab Đăng ký, còn lại mặc định Đăng nhập
  ...
  const [loiChung, setLoiChung] = useState(""); // lỗi trả về từ Backend, vd: sai mật khẩu
    ...
    <div className="min-h-screen bg-gradient-to-b from-[#FDEBC2] via-[#FDF3DA] to-[#FEFAF1] font-['Be_Vietnam_Pro']">
      ...
      <div className="max-w-5xl mx-auto px-5 py-10 md:py-16">
        <div className="grid md:grid-cols-2 gap-10 items-center">
          {/* ============ CỘT TRÁI: MINH HỌA (chỉ hiện trên màn hình lớn) ============ */}
