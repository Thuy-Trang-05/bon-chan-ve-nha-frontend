/**
 * TimKiem.jsx — Giao diện trang "Tìm kiếm và danh sách tin đăng"
 * -----------------------------------------------------------------
 * Trang này gộp 3 use case liên quan ở Chương 3 (vì cùng một luồng
 * "xem danh sách tin" của người dùng):
 *   - UC-12: Tìm kiếm và lọc tin đăng   (tác nhân: Khách, Người dùng)
 *   - UC-11: Xem vị trí tin đăng trên bản đồ (tác nhân: Khách, Người dùng)
 *   - UC-10: Xem thông tin bài đăng     (tác nhân: Khách, Người dùng)
 *     (UC-10 ở đây chỉ có nút "Xem chi tiết" dẫn sang trang riêng,
 *      trang Chi tiết tin đăng sẽ làm ở bước sau)
 *
 * Vì cả Khách lẫn Người dùng đều xem được trang này nên KHÔNG cần
 * kiểm tra đăng nhập ở đây (khác với trang Đăng tin).
 *
 * Ghi chú tích hợp API thật:
 *   - Bộ lọc nên gọi API dạng:
 *       axios.get("/api/tin-dang", { params: { loai_thu_cung, khu_vuc,
 *         trang_thai, tu_khoa, trang: soTrang } })
 *   - Khối "Bản đồ" hiện là bản rút gọn (ghim vị trí bằng % tọa độ giả
 *     lập để xem giao diện). Trong dự án thật thay bằng component bản đồ
 *     của thư viện bản đồ tương tác (kết hợp OpenStreetMap) với marker lấy từ trường lat/lng thật của mỗi
 *     tin đăng (đã trình bày ở mục 2.3.4), vì thư viện bản đồ tương tác (kết hợp OpenStreetMap) không dùng
 *     được trong môi trường xem trước này.
 */
...
// Dữ liệu mẫu — sẽ thay bằng kết quả thật lấy từ GET /api/tin-dang
// (lat/lng là toạ độ thật gần đúng các khu vực ở Huế, dùng để nhúng
// bản đồ Google Maps thật cho từng tin khi được chọn)
const MOCK_TIN_DANG = [
...
export default function TimKiem() {
  ...
  const [cheDoXem, setCheDoXem] = useState("danh_sach"); // "danh_sach" | "ban_do"
  const [boLocMoRong, setBoLocMoRong] = useState(false);
  const [tinDuocChon, setTinDuocChon] = useState(null); // dùng khi xem trên bản đồ
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-[#FDEBC2] via-[#FDF3DA] to-[#FEFAF1] font-['Be_Vietnam_Pro']">
      ...
      <div className="max-w-6xl mx-auto px-5 py-8">
        ...
        {/* ============ THANH KẾT QUẢ + CHUYỂN CHẾ ĐỘ XEM ============ */}
        ...
        {ketQua.length > 0 && cheDoXem === "ban_do" && (
          <div className="grid md:grid-cols-3 gap-5">
            {/* Đây CHỈ là hình minh họa vị trí các tin để xem bố cục
               giao diện — không phải bản đồ thật.

               CODE THẬT của dự án phải dùng thư viện bản đồ kết hợp
               OpenStreetMap (đúng như đã chọn ở mục 2.3.4 bài tiểu
               luận), hiển thị TẤT CẢ ghim cùng lúc trên 1 bản đồ. Đoạn
               code mẫu để thay khối div bên dưới, Claude đã gửi riêng
               ở khung chat, không đặt trong file này để tránh công cụ
               xem trước từ chối tải artifact. */}
            ...
            {/* Panel thông tin tin đăng được chọn trên bản đồ */}
            <div>
              {tinDuocChon ? (
